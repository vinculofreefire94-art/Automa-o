...
# ─────────────────────────────────────────────────────────────────────────────
#  Paleta de Cores — Dark Trading Terminal
# ─────────────────────────────────────────────────────────────────────────────

PALETTE = {
    "bg_main":       "#0A0E1A",   # Azul-preto profundo
    "bg_panel":      "#0F1629",   # Painéis
    "bg_chart":      "#0B1120",   # Área do gráfico
    "grid":          "#1A2040",   # Grid sutil
    "text_primary":  "#E8ECFF",   # Texto principal
    "text_secondary": "#6B7A99",  # Texto secundário
    "text_accent":   "#A0B4FF",   # Destaque

    "bull_body":     "#00E676",   # Verde neon candle
    "bull_wick":     "#00E676",
    "bear_body":     "#FF3D57",   # Vermelho neon candle
    "bear_wick":     "#FF3D57",

    "ema9":          "#FFD600",   # Amarelo
    "ema21":         "#FF6D00",   # Laranja
    "ema50":         "#00B0FF",   # Azul claro
    "ema200":        "#AA00FF",   # Roxo
...
# ─────────────────────────────────────────────────────────────────────────────
#  Gerador de Gráfico Elite
# ─────────────────────────────────────────────────────────────────────────────

class EliteChartGenerator:
    """Gera gráficos profissionais multi-painel para sinais de trading."""
    ...
    def generate(self, signal: TradingSignal, df: pd.DataFrame) -> Optional[bytes]:
        """Gera o gráfico completo e retorna bytes PNG."""
        try:
            # Últimos N candles para o gráfico
            ...
            # Grid Layout: preço(6) + MACD(2) + RSI(2) + Volume(2)
            ...
            # Oculta xtick do preço, macd e rsi (exceto último)
    ...
    def _draw_signal_levels(self, ax, signal: TradingSignal, df: pd.DataFrame):
        ...
        # Zona colorida entre entry e TP
        ...
        # Seta de sinal na última vela
        arrow_y_start = signal.entry
        dy = (signal.take_profit - signal.entry) * 0.3
        ax.annotate(
            "",
            xy=(x_end, arrow_y_start + dy),
            xytext=(x_end, arrow_y_start - dy),
            arrowprops=dict(
                arrowstyle="->",
                color=color,
                lw=2,
    ...
    def _draw_macd(self, ax, df: pd.DataFrame):
        ...
        if "macd" not in df.columns:
            ax.text(0.5, 0.5, "MACD N/D", transform=ax.transAxes,
                    ha="center", color=PALETTE["text_secondary"])
            return

        x = np.arange(len(df))
        hist = df["macd_hist"].fillna(0).values
        macd_line = df["macd"].values
        sig_line = df["macd_signal"].values
        ...
        ax.plot(x, macd_line, color=PALETTE["macd_line"], linewidth=1.2,
                label="MACD", zorder=3)
        ax.plot(x, sig_line, color=PALETTE["macd_signal"], linewidth=1.0,
                linestyle="--", label="Signal", zorder=3)
    ...
    def _draw_rsi(self, ax, df: pd.DataFrame):
        ...
        # Fill zonas
        ax.fill_between(x, rsi, 30, where=(rsi < 30),
                        color=PALETTE["rsi_os"], alpha=0.2)
        ax.fill_between(x, rsi, 70, where=(rsi > 70),
                        color=PALETTE["rsi_ob"], alpha=0.2)
    ...
    def _format_x_axis(self, ax, df: pd.DataFrame):
        # Ticks customizados baseados no índice
        n = len(df)
        step = max(1, n // 8)
    ...
    def _draw_header(self, fig, signal: TradingSignal):
        ...
        title = (
            f"  {dir_emoji}  ·  Score {signal.score:.0f}/100  ·  "
            f"Força [{strength_bar}]  ·  {phase}  ·  "
            f"R/R {signal.rr_ratio:.2f}x  "
        )
    ...
    def _draw_footer(self, fig, signal: TradingSignal):
        ...
        fig.text(
            ...
            ha="center", va="bottom",
            fontsize=6.5, color=PALETTE["text_secondary"],
            fontfamily=FONT_FAMILY,
        )
